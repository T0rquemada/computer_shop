...
function cart_exist(int $user_id) : bool {
    ...
    if (!isset($user_id)) {
        throw new Exception("'user_id' not provided while checking that cart exist!");
    }
...
function create_cart(int $user_id, $items): void {
    global $pdo;

    if (!isset($user_id, $items)) {
        throw new Exception("'user_id' or items not provided while creating cart!");
    }
        ...
        case '/updatecart': 
            ...
            if ($correct_data) {
                if (cart_exist($cart_data['user_id'])) {
                    ...
                    echo json_encode(['message' => 'Update existing cart']);
                } else {
                    create_cart($cart_data['user_id'], $cart_data['item']);
                    echo json_encode(['message' => 'Create cart']);
                }
            } else {
                http_response_code(400);
                echo json_encode(['message' => 'Incorrect json cart data while updating cart!']);
            }            
            break;
        default:
            http_response_code(400);
            echo json_encode(['message' => 'POST-route not found']);
            break;
    }
