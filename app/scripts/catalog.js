...
async function get_items(route) {
    let requestData = { route: `catalog.php/${route}`, method: 'GET'}
    let items = await request(requestData);
    return items
}
...
async function saveToCart(item) {
    ...
    let body = { user_id: userId, item: item };

    let requestData = { route: 'cart.php/updatecart', method: 'POST', body: body}
